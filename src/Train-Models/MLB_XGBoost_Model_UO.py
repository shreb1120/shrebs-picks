"""Train XGBoost over/under model for MLB with walk-forward CV."""
...
DEFAULT_DATASET = "mlb_dataset"
TARGET_COLUMN = "OU_Cover"
DATE_COLUMN = "Date"
DROP_COLUMNS = [
...
def prepare_data(df):
    ...
    # Filter out rows without OU data (NaN) and push rows (class 2)
...
def train_model(X_train, y_train, X_val, y_val, params, num_boost_round):
    dtrain = xgb.DMatrix(X_train, label=y_train, weight=compute_sample_weights(y_train, NUM_CLASSES))
...
def main():
    parser = argparse.ArgumentParser(description="Train MLB XGBoost totals model.")
    ...
    params = best["params"]
    model_name = (
        f"MLB_XGBoost_{accuracy * 100:.1f}%_UO"
